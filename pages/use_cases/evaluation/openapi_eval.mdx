...
### 可选：生成输入问题并请求地面实况查询 [#](#optional-generate-input-questions-and-request-ground-truth-queries "Permalink to this headline")
...
1. 将用户查询转换为API请求（请求合成器）(request synthesizer)
2. 将API响应转换为自然语言响应
...
首先，我们将解析OpenAPI规范。在这个例子中，我们将使用Speak的OpenAPI规范[Speak](https://www.speak.com/) 。
...
**现在，您可以使用上面在评估请求链中显示的 `ground_truth` ！** 
