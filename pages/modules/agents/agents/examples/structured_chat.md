...
较旧的代理被配置为将操作输入指定为单个字符串，但是该代理可以使用所提供的工具的 `args_schema` 来填充操作输入。


此功能在（ `structured-chat-zero-shot-react-description` 或 `AgentType.STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION` ）中可用。
