...
```
```
# We're using the default `documents` table here. You can modify this by passing in a `table\_name` argument to the `from\_documents` method.
vector\_store = SupabaseVectorStore.from\_documents(
